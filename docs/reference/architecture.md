...
function generatePaymentId(storeId: string, orderId: string): `0x${string}` {
  return keccak256(
    concat([
      toBytes(storeId),      // Extracted from API Key
      toBytes(orderId),      // Store server request
      randomBytes(32)        // Ensures unique ID every time
    ])
  );
}
```
...
**Characteristics**:
- Same store + same orderId → Always different paymentId
- Includes storeId → No collision between stores
- Includes randomBytes → New ID for identical requests
