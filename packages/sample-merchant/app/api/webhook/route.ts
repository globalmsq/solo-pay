...
export async function POST(request: NextRequest) {
  try {
    ...
    if (gatewayPayment.status !== 'CONFIRMED') {
      ...
      return NextResponse.json(
        { error: 'Payment not confirmed on gateway' },
        { status: 400 }
      );
    }
    ...
    const gatewayAmount =
      Number(BigInt(gatewayPayment.amount)) /
      10 ** gatewayPayment.tokenDecimals;
    const localAmount = Number(localPayment.amount);

    if (gatewayAmount !== localAmount) {
      console.error(
        `[webhook] Amount mismatch: local=${localAmount}, gateway=${gatewayAmount}`
      );
      return NextResponse.json(
